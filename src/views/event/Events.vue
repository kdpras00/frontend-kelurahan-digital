...
const applyFilters = (newFilters) => {
  // Handle range filter untuk price
  if (newFilters.price) {
    filters.value.price_min = newFilters.price.min;
    filters.value.price_max = newFilters.price.max;
  }
  
  filters.value = {
    ...filters.value,
    ...newFilters,
    price: undefined, // Remove price object, use price_min/max instead
  };
...
<template>
  ...
  <section id="List-Event-Desa" class="flex flex-col gap-[14px]">
    ...
    <!-- Data List -->
    <template v-else>
      <CardList v-for="event in events" :key="event.id" :item="event" />
      ...
      >
        ...
        <p class="text-lg font-medium text-desa-secondary">
          Tidak ada event desa
        </p>
        <p class="text-sm text-desa-secondary mt-2">
          {{ filters.search ? 'Coba dengan kata kunci lain' : 'Belum ada event tersedia' }}
        </p>
      </div>
    </template>
  ...
  <FilterModal
    ...
    :filters="{ ...filters, price: { min: filters.price_min, max: filters.price_max } }"
