...
const fetchData = async () => {
  ...
  if (data) {
    profile.value = {
      ...
      agricultural_area: data.agricultural_area || null,
      total_area: data.total_area || null,
      images: [], // For additional images if user wants to add more
    };
  }
};
...
const handleFileChange = (index, event) => {
  ...
  if (file) {
    profile.value.images[index].image = file;
    profile.value.images[index].image_url = URL.createObjectURL(file);
  }
};
      ...
      <section id="Photos" class="flex justify-between min-w-0">
        ...
        <div class="photo-input-container flex flex-col gap-6 flex-1 min-w-0">
          <div class="photo-form group/parent flex items-center justify-between">
            ...
            >
              <img
                v-if="profile.thumbnail_url"
                class="Photo size-full object-cover"
                :src="profile.thumbnail_url"
                alt="image"
              />
              <span v-else class="text-xs text-desa-secondary text-center p-2">Belum ada thumbnail</span>
            </div>
            <div class="relative">
              <input
                type="file"
