<script setup>
import { can, isRole } from '@/helpers/permissionHelper'
...
const shouldShowItem = computed(() => {
    ...
    if (!can(props.item.permission)) return false
    
    // Check if item should be hidden for current role
    if (props.item.hideForRoles && Array.isArray(props.item.hideForRoles)) {
        // Check if current user has any of the roles that should hide this item
        const userShouldHide = props.item.hideForRoles.some(role => isRole(role))
        if (userShouldHide) {
            return false
        }
    }
...
<template>
    <li class="group" :class="{ active: isActive }" v-if="!item.children && shouldShowItem">
